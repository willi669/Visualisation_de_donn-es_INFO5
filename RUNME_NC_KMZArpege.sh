...
NomDuFichierMeteoFrance=`python PYTHON/RequeteArpege.py $1 SP1`
rc=$?
if ! [ $rc == 0 ]; then
...
DateDeLaPrevision=`python PYTHON/DateDeLaPrevisionArpege.py $1`
FinDeLaPrevision=`python PYTHON/FinDeLaPrevisionArpege.py $1`
DateDuRun=`python PYTHON/DateDuRunArpege.py $1`
...
#	wgrib2 DATA/$NomDuFichierMeteoFrance -match ":TMP:" -netcdf DATA/$NomDuFichierMeteoFrance.nc
	...
	/home/willeiam/S9/Visu/Sceance_10Novembre/wgrib2_execbin/wgrib2 DATA/$NomDuFichierMeteoFrance -match ":(UGRD|VGRD|TMP):" -netcdf DATA/$NomDuFichierMeteoFrance.nc
...
if [ -e KML/IMAGES/$NomDuFichierMeteoFrance.nc.png ]; then
	...
	/home/willeiam/ParaView-5.8.1-MPI-Linux-Python2.7-64bit/bin/pvpython --force-offscreen-rendering PYTHON/VisuAvecTemperatureArpege.py DATA/$NomDuFichierMeteoFrance.nc
...
echo
cat KML/templateSpanKMZArpege.kml | sed "s/ICILEFICHIER/$NomDuFichierMeteoFrance.nc.png/g" > tmp.kml
