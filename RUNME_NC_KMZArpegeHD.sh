...
NomDuFichierMeteoFrance=`python PYTHON/RequeteArpegeHD.py $1 SP1`
rc=$?
if ! [ $rc == 0 ]; then
...
DateDeLaPrevision=`python PYTHON/DateDeLaPrevisionArpegeHD.py $1`
FinDeLaPrevision=`python PYTHON/FinDeLaPrevisionArpegeHD.py $1`
DateDuRun=`python PYTHON/DateDuRunArpegeHD.py $1`
...
if [ -e KML/IMAGES/$NomDuFichierMeteoFrance.nc.png ]; then
	...
	pvpython --force-offscreen-rendering PYTHON/VisuAvecTemperatureArpegeHD.py DATA/$NomDuFichierMeteoFrance.nc
...
echo
cat KML/templateSpanKMZArpegeHD.kml | sed "s/ICILEFICHIER/$NomDuFichierMeteoFrance.nc.png/g" > tmp.kml
